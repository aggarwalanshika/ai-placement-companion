...
export default function ResumeHistory() {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-10 relative select-none">
      ...
      {isRescanning && (
        <div className="p-4 bg-slate-950/60 border border-slate-800 rounded-xl flex items-center gap-3 text-xs text-slate-300">
      ...
      <div className="space-y-4">
        {versions.length === 0 ? (
          <div className="p-12 border border-slate-850 bg-slate-900/10 rounded-2xl text-center space-y-4 max-w-md mx-auto mt-8">
            <AlertCircle className="mx-auto h-8 w-8 text-slate-500" />
            <div>
              <h3 className="text-sm font-semibold text-white">No resume versions yet</h3>
          ...
          versions.map((item, idx) => (
            <motion.div
              ...
              className="p-5 bg-slate-900/10 border border-slate-850 rounded-2xl shadow-lg flex flex-wrap items-center justify-between gap-4 text-xs"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="space-y-1">
                  <span className="font-bold text-white block leading-tight">{item.fileName || 'resume.pdf'}</span>
                  <div className="flex flex-wrap items-center gap-3 text-[10px] text-slate-500 font-medium">
                    ...
                    <span>•</span>
                    <span className="text-indigo-400 font-semibold">{item.userNotes || 'Optimized Draft'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex gap-1.5">
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 bg-slate-900 border border-slate-850 hover:bg-slate-800 text-slate-300 text-[10px] font-bold rounded-lg transition-all disabled:opacity-50"
                  >
                    <Sparkles className="w-3.5 h-3.5 text-indigo-400" /> Re-scan
                  </button>
                  <button
                  ...
                  <button
                    ...
                    disabled={isRescanning}
                    className="flex items-center gap-1 px-3 py-1.5 bg-slate-950 border border-slate-900 hover:bg-slate-900 text-slate-350 text-[10px] font-bold rounded-lg transition-all disabled:opacity-50"
                  >
